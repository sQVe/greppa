...
export const useFileSelectionHandlers = ({
  ...
  const committedFilePaths = useMemo(() => collectFiles(files).map((file) => file.path), [files]);
  const worktreeFilePaths = useMemo(
    () => collectFiles(worktreeFiles).map((file) => file.path),
    [worktreeFiles],
  );
  const allCommittedFilePaths = useMemo(() => new Set(committedFilePaths), [committedFilePaths]);

  const allWorktreeFilePaths = useMemo(() => new Set(worktreeFilePaths), [worktreeFilePaths]);
  ...
  const handleSelectCommittedFile = useCallback((path: string, modifiers: SelectModifiers) => {
  ...
  const handleSelectCommittedDirectory = useCallback((path: string) => {
    const current = refs.current;
    const children = collectDescendantFilePaths(current.files, path);
    if (children.length > 0) {
  ...
  const handleSelectWorktreeFile = useCallback((path: string, modifiers: SelectModifiers) => {
    const current = refs.current;
    if (modifiers.shiftKey) {
  ...
  const handleSelectWorktreeDirectory = useCallback((path: string) => {
    const current = refs.current;
    const children = collectDescendantFilePaths(current.worktreeFiles, path);
    if (children.length > 0) {
